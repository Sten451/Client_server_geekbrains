...
from common.variables import ACCOUNT_LOGIN, USER,CURRENT_TIME, ACTION,DEF_PORT, DEF_IP_ADDRESS, \
    MAX_CONNECTIONS, CODE_RESPONSE, CODE_ERROR, CODE_PRESENCE, MESSAGE, MESSAGE_TEXT, SENDER
...
def process_client_message(message, messages_list, client):
    ...
    if ACTION in message and message[ACTION] == CODE_PRESENCE and CURRENT_TIME in message \
        ...
        send_message(client, {CODE_RESPONSE: 200})
        return
    # Если это сообщение, то добавляем его в очередь сообщений. Ответ не требуется.
    elif ACTION in message and message[ACTION] == MESSAGE and \
            CURRENT_TIME in message and MESSAGE_TEXT in message:
        messages_list.append((message[ACCOUNT_LOGIN], message[MESSAGE_TEXT]))
        return
